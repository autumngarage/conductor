...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --consumer-paths)
      shift
      [ "$#" -gt 0 ] || { echo "ERROR: --consumer-paths requires a value" >&2; exit 1; }
      consumer_paths="$1"
      ;;
    --consumer-config)
      shift
      [ "$#" -gt 0 ] || { echo "ERROR: --consumer-config requires a value" >&2; exit 1; }
      consumer_config="$1"
      ;;
    --consumer-branch)
      shift
      [ "$#" -gt 0 ] || { echo "ERROR: --consumer-branch requires a value" >&2; exit 1; }
      consumer_branch="$1"
      ;;
    --no-auto-stash)
